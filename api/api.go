...
func Setup(router *mux.Router,
	...
	v1.HandleFunc("/search-reindex-jobs/{id}/number_of_tasks/{count}", permissions.Require(update, api.PutNumTasksHandler)).Methods("PUT")
...
func (api *API) v1RoutesAsLatest(permissions AuthHandler) {
	...
	api.Router.HandleFunc("/search-reindex-jobs/{id}/number_of_tasks/{count}", permissions.Require(update, api.PutNumTasksHandler)).Methods("PUT")
