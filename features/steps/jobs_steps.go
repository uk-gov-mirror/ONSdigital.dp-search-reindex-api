...
const validServiceAuthToken = "abcdefg"
const validUserToken = "hijklmn"
const invalidServiceAuthToken = "opqrstu"
const invalidUserToken = "wxyz"
...
func (f *JobsFeature) RegisterSteps(ctx *godog.ScenarioContext) {
	...
	ctx.Step(`^I use a valid X Florence user token$`, f.iUseAValidXFlorenceUserToken)
	ctx.Step(`^I use an invalid X Florence user token$`, f.iUseAnInvalidXFlorenceUserToken)
}

//IAmAuthorised sets the Authorization header to use a fake SERVICE_AUTH_TOKEN value. The fake Zebedee service will recognise this as being valid.
func (f *JobsFeature) IAmAuthorised() error {
	f.ApiFeature.ISetTheHeaderTo("Authorization", validServiceAuthToken)
	return nil
}

//iUseAnInvalidServiceAuthToken sets the Authorization header to use a fake SERVICE_AUTH_TOKEN value. The fake Zebedee service will NOT recognise this as being valid.
...
//iUseAValidXFlorenceUserToken sets the Authorization header to use a fake X-Florence-Token value. The fake Zebedee service will recognise this as being valid.
func (f *JobsFeature) iUseAValidXFlorenceUserToken() error {
	...
	f.AuthFeature.FakeAuthService.NewHandler().Get("/userInstancePermissions").Reply(200).BodyString(`{ "permissions": ["DELETE", "READ", "CREATE", "UPDATE"]}`)
	return nil
}

//iUseAnInvalidXFlorenceUserToken sets the Authorization header to use a fake X-Florence-Token value. The fake Zebedee service will NOT recognise this as being valid.
func (f *JobsFeature) iUseAnInvalidXFlorenceUserToken() error {
	f.ApiFeature.ISetTheHeaderTo("X-Florence-Token", invalidUserToken)
	f.AuthFeature.FakeAuthService.NewHandler().Get("/identity").Reply(401).BodyString(`{ "message": "user not authenticated"}`)
	f.AuthFeature.FakeAuthService.NewHandler().Get("/userInstancePermissions").Reply(401).BodyString(`{ "message": "CMD permissions request denied: session not found"}`)
	return nil
}
